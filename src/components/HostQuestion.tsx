...
//   const updateGameState = async (updates: Record<string, any>) => {
//     try {
...
//   useEffect(() => {
//     if (!gameCode || questionIndex === undefined) return;

//     const fetchGameAndQuiz = async () => {
//       try {
//         const gameRes = await fetch(`http://localhost:5000/games/${gameCode}`);
//         if (!gameRes.ok) throw new Error('Game not found');
//         const gameData = await gameRes.json();

//         if (!gameData?.quizId) throw new Error('quizId missing from game');
...
//         const currentQuestion = quizData.questions[qIndex];
...
//         await updateGameState({ questionIndex: qIndex, showScoreboard: false });
//       } catch (err) {
...
//       setScoreboardData(res.data as PlayerScore[]);
...
//       await updateGameState({ questionIndex: nextIndex, showScoreboard: false });
...
//   );
// }
// src/components/HostQuestion.tsx
// src/components/HostQuestion.tsx
import { useParams, useNavigate } from 'react-router-dom';
import { useEffect, useState } from 'react';
import QuestionCard from './QuestionCard';
