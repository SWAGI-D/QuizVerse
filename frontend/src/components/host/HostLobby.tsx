...
const HostLobby: FC = () => {
  // Destructure gameCode without a Params interface to avoid TS constraint issues
  ...
  // Build the invite link for sharing
  const inviteLink = `${window.location.origin}/join?gameCode=${gameCode}`;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.error('Error loading quiz title:', err);
      }
    })();
  ...
  // Copy just the game code to clipboard
  const copyCode = () => {
    navigator.clipboard.writeText(gameCode);
    alert('Game code copied!');
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0f0c29] via-[#302b63] to-[#24243e] text-white px-6 py-10 flex items-center justify-center">
      <div className="w-full max-w-3xl bg-white/10 backdrop-blur-md p-8 rounded-2xl shadow-2xl">
        ...
        <p className="text-lg text-center mb-4">Or send this invite link:</p>
        <div className="flex justify-center items-center gap-4 mb-6">
          <input
            type="text"
            readOnly
            value={inviteLink}
            className="w-full bg-white/20 text-white px-4 py-2 rounded-xl focus:outline-none"
          />
          <button
